 ...
 * @ide     Keil uVision
 * @license GNU GPL v3
 * @brief   GPIO Library for STM32F1xx devices
 *
@verbatim
   ----------------------------------------------------------------------
...
#else
# define TOTAL_PORTS   11
#endif

#define getGpioIndex(gpio)                                                  \
            (((uint32_t)gpio - (GPIOA_BASE)) / ((GPIOB_BASE) - (GPIOA_BASE)))
#define gpioIsPinInput(mode)    ((mode & 0x3) == 0)
#define gpioIsPinOutput(mode)   !gpioIsPinInput(mode)
#define gpioIsPinAltFunc(mode)  (((mode >> 2) & 0x3) >= 2)
/**
 * Get the correct GPIO Control Register for the given `pin'. The `pin' is
 * also properly adjusted.
 */
#define getGpioControlReg(gpio, pin)                                        \
                            (pin > 8? (pin -= 8, &gpio->CRH):&gpio->CRL)
